...
import ch.ethz.systems.strymon.ds2.flink.nexmark.sources.NexmarkDynamicBatchSourceFunction;
import ch.ethz.systems.strymon.ds2.flink.nexmark.sources.dirigo.FunctionInvocation;
import ch.ethz.systems.strymon.ds2.flink.nexmark.sources.dirigo.GeneratorConfig;
import ch.ethz.systems.strymon.ds2.flink.nexmark.sources.dirigo.InternalTypedSourceObject;

import ch.ethz.systems.strymon.ds2.flink.nexmark.sources.dirigo.NexmarkConfiguration;
...
//        })
                ...
                .aggregate(new AggregateFunction<Tuple2<String, Object>, Tuple2<Long, Bid>, Tuple2<Long, Bid>>() {
                    @Override
                    public Tuple2<Long, Bid> createAccumulator() {
                        return new Tuple2<>(0L, null);
                    }

                    @Override
                    public Tuple2<Long, Bid> add(Tuple2<String, Object> o, Tuple2<Long, Bid> longBidTuple2) {
                        if(o.f1 instanceof ArrayList){
                            ...
                            for(Bid bid : (ArrayList<Bid>)o.f1){
                                if(bid.price > highestBid.price) highestBid = bid;
                                if(bid.dateTime > highestTS) highestTS = bid.dateTime;
                            }
                            Tuple2<Long, Bid> ret =  new Tuple2<>(highestTS, highestBid);
                    ...
                    @Override
                    public Tuple2<Long, Bid> getResult(Tuple2<Long, Bid> accumulator) {
                        System.out.println("getResult accumulator " + accumulator);
                        return accumulator;
                    }

                    @Override
                    public Tuple2<Long, Bid> merge(Tuple2<Long, Bid> acc1, Tuple2<Long, Bid> acc2) {
        ...
        bidDataStream.addSink(new RichSinkFunction<Tuple2<Long, Bid>>() {
            @Override
            public void invoke(Tuple2<Long, Bid> value, Context context) throws Exception {
